...
**Q:** How to interpret the figures reported under "Proportions"? Is a fatal case also counted as critical or severe case? Similarly, is a critical case also counted as severe case?

**A:** _A: These numbers sum to 100%. Critical is hence the fraction of people that fall critically ill and didn't die. Severe are those that get severely ill but don't need critical care etc._

---

**Q:** As the individual becomes "fractional" along the path I assume only the fraction that achieved the worst status is counted in the "Proportions". Is an individual that runs through the state model only counted in the worst state the individual achieves?

**A:** _We count the most serious outcome for a case._
