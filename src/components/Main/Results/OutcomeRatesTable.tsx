...
export function OutcomeRatesTable({ showHumanized, result, rates, printable }: TableProps) {
  ...
  */
  ...
  const endResult = result.trajectory.middle[result.trajectory.middle.length - 1]
  ...
  const peakSevere = Math.round(Math.max(...result.trajectory.middle.map((x) => x.current.severe.total)))
  const peakCritical = Math.round(Math.max(...result.trajectory.middle.map((x) => x.current.critical.total + x.current.overflow.total))) // prettier-ignore
