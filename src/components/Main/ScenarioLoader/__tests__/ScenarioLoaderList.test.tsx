...
function renderScenarioList({ scenarioNames, setScenario, close = noop }: Optional<ScenarioLoaderListProps> = {}) {
  return render(<ScenarioLoaderList close={close} />)
}
...
describe('ScenarioLoaderList', () => {
  ...
  describe('selects clicked scenario', () => {
    it('when list is not filtered', () => {
      ...
      const { getByText } = renderScenarioList()
    ...
    it('when list is filtered', async () => {
      const onScenarioSelect = jest.fn<void, [string]>()
      const { getByPlaceholderText, getByText } = renderScenarioList()
      const input = getByPlaceholderText('Search')
      await userEvent.type(input, 'ita')
