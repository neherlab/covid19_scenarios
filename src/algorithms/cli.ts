...
import type { ScenarioFlat, ScenarioData } from './types/Param.types'
...
async function main() {
  ...
  const severity = getSeverityDistributionData(DEFAULT_SEVERITY_DISTRIBUTION).data
  // Read data from ageDistribution.json and perform JSON validation.
  console.log(`Reading age distribution data`)
  const ageDistribution = getAgeDistributionData(params.ageDistributionName).data
